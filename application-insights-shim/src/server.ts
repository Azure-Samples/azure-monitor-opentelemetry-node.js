...
function handleRequest(request: any, response: any) {
  ...
  request.on("end", () => {
    ...
    if (request.url == '/') {
    ...
    else if (request.url == '/trace') {
      appInsightsClient.trackTrace({message: "trace message"});
    }
    else if(request.url == '/dependency') {
      appInsightsClient.trackDependency({target: "http://testdependency.com", name: "GET http://testdependency.com"});
    }
    else if (request.url == '/request') {
      appInsightsClient.trackRequest({name: "GET /testrequest", url: "http://testrequest.com", duration: 309, resultCode: 200, success: true});
    }
      ...
      appInsightsClient.trackMetric({ name: "custom_metric", value: 42 });
    }
